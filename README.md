...
**Phase 5 — 월드·연출 완료.** 올라갈수록 배경이 바뀌고(숲 → 저녁 숲 → 성 → 밤의 성),
10층마다 체크포인트가 뜨고, 펫이 따라온다. 학습 엔진·단어 DB 455개(L1~5)는 Phase 4 에서 완료.

남은 것: **보급형 태블릿 실기기 30fps 실측**, 태블릿 가로 2단 레이아웃,
단어 Level 6~10 확장(목표 1,000개 중 45%), Phase 6(성장·미션·부모 화면).
